...
Para mitigar el desbalance extremo de los datos (96% de hardware sano vs 4% de fallos), se configuro el hiperparametro `class_weight='balanced'`, forzando a la maquina a penalizar severamente los falsos negativos (es decir, evitar a toda costa predecir que un equipo esta bien cuando en realidad esta por fallar).
...
* `matplotlib` & `seaborn`: Analisis Exploratorio (EDA) y renderizado del hiperplano 2D.
...
2. **Abrir el entorno:** Importa el archivo `.ipynb` en Google Colab.
